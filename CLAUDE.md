...
**Dust Off** — a randomized photo gallery and slideshow app for iOS and Android. The wedge is _serendipitous rediscovery_ (random-by-default sort + a dedicated, gesture-first slideshow) for users with 5k–50k photos who never scroll back through their library.
...
- Shuffled-queue invariant: no repeats until queue exhausted; reshuffle silently at end (S-5); shuffle history is _not_ persisted across sessions (S-6)
...
| 6     | Slideshow engine — autoplay, transitions, lifecycle                           | _(working tree, uncommitted)_  |
